...
import { Users, Clock, Calendar, QrCode, Info, CircleCheck as CheckCircle, TriangleAlert as AlertTriangle, Circle as XCircle, Trash2, MoveHorizontal as MoreHorizontal, UserPlus, MapPin, Bell } from 'lucide-react-native';
...
export default function NotificationItem({ notification, onPress }: NotificationItemProps) {
  ...
  const getStatusIcon = () => {
    if (notification.type === 'user_invite' || notification.type === 'join_request') {
      ...
      switch (status) {
        case 'approved':
          return { icon: CheckCircle, color: '#42b883' };
        case 'declined':
        case 'rejected':
          return { icon: XCircle, color: '#e74c3c' };
        default:
          return { icon: Clock, color: '#f39c12' };
      }
    }
    ...
    if (notification.type === 'attendance') {
      ...
      switch (status) {
        ...
        case 'present':
          return { icon: CheckCircle, color: '#42b883' };
        case 'late':
          return { icon: AlertTriangle, color: '#ff6b35' };
        case 'error':
        case 'invalid_qr':
          return { icon: XCircle, color: '#e74c3c' };
        default:
          return { icon: Clock, color: '#1877f2' };
      }
    }
    ...
    >
      ...
      <View style={styles.leftSection}>
        <View style={[styles.avatarContainer, { backgroundColor: avatarBackground }]}>
          <NotificationIcon size={20} color={iconColor} strokeWidth={2.5} />
          {statusIcon && (
            <View style={[styles.statusBadge, { backgroundColor: statusIcon.color }]}>
              <statusIcon.icon size={10} color="#ffffff" strokeWidth={3} />
            </View>
          )}
        </View>
        ...
        {notification.metadata && (
          <View style={styles.metadataContainer}>
            {notification.type === 'user_invite' && notification.targetAudience === 'admin' && (
              <View style={styles.metadataRow}>
                <View style={styles.metadataChip}>
                  <Text style={styles.metadataText}>{notification.metadata.userEmail}</Text>
                </View>
            ...
            {notification.type === 'join_request' && notification.targetAudience === 'user' && (
              <View style={styles.metadataRow}>
                <View style={styles.metadataChip}>
                  <Text style={styles.metadataText}>{notification.metadata.organizationName}</Text>
                </View>
                <View style={styles.metadataChip}>
                  <Text style={styles.metadataText}>by {notification.metadata.adminName}</Text>
                </View>
              </View>
            )}
              ...
              <View style={styles.metadataRow}>
                {notification.metadata.location && (
                  <View style={styles.metadataChip}>
                    ...
                    <Text style={styles.metadataText}>{notification.metadata.location}</Text>
                  </View>
                )}
                {notification.metadata.checkInTime && (
                  <View style={styles.metadataChip}>
                    <Clock size={10} color="#6b7280" />
                    <Text style={styles.metadataText}>{notification.metadata.checkInTime}</Text>
                  </View>
                )}
              </View>
            )}
            ...
            {notification.type === 'qr_management' && notification.targetAudience === 'admin' && (
              <View style={styles.metadataRow}>
                <View style={styles.metadataChip}>
                  <Text style={styles.metadataText}>{notification.metadata.qrCodeType}</Text>
                </View>
                <View style={styles.metadataChip}>
                  <Text style={styles.metadataText}>{notification.metadata.usageCount} scans</Text>
                </View>
              </View>
            )}
          </View>
