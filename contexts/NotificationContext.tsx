...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addNotification = (notification: Omit<AppNotification, 'id' | 'createdAt'>) => {
    FirebaseService.createNotification(notification).catch(console.error);
  };

  const markAsRead = (notificationId: string) => {
    FirebaseService.markNotificationRead(notificationId).catch(console.error);
  };

  const markAllAsRead = (audience?: 'admin' | 'user') => {
    if (!user) return;
    if (audience && audience !== (user.role === 'user' ? 'user' : 'admin')) return;
    FirebaseService.markAllNotificationsRead(user.id, user.role).catch(console.error);
  };

  const deleteNotification = (notificationId: string) => {
    FirebaseService.deleteNotification(notificationId).catch(console.error);
  };

  const clearAllNotifications = (audience?: 'admin' | 'user') => {
    const filtered = notifications.filter((n) => !audience || n.targetAudience === audience);
    filtered.forEach((n) => FirebaseService.deleteNotification(n.id).catch(console.error));
  };
